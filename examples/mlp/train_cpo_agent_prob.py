...
from fsrl.agent import CPOAgent_PROB
...
def train(args: TrainCfg):
    ...
    if __name__ == "__main__":
        ...
        collector = FastCollector(agent.policy, env, args.cost_limit)
...
@pyrallis.wrap()
def train2(args: TrainCfg):

    # update config
    cfg, old_cfg = asdict(args), asdict(TrainCfg())
    differing_values = {key: cfg[key] for key in cfg.keys() if cfg[key] != old_cfg[key]}
    cfg = asdict(TASK_TO_CFG[args.task]())
    print(f"Configurations are ccc: {cfg}")
    cfg.update(differing_values)
    args = types.SimpleNamespace(**cfg)
    ...
    if __name__ == "__main__":
        # Let's watch its performance!
        from fsrl.data import FastCollector, FastCollector_PROB
        env = gym.make(args.task)
        agent.policy.eval()
        collector = FastCollector_PROB(agent.policy, env)
        # modification: cost_limit = 1
        result = collector.collect(n_episode=10, render=args.render, cost_limit = 1)
        rews, lens, cost = result["rew"], result["len"], result["cost"]
        print(f"Final eval reward: {rews.mean()}, cost: {cost}, length: {lens.mean()}")
